*
* SIMPLE PIPELINE:
*   1) Set revised paths and run controls for 2018 workflow.
*   2) Rebuild requirement and consumption components with updates.
*   3) Re-estimate CV/PoU using revised assumptions.
*   4) Save revised output tables for comparison.
*
...
if "`c(username)'" == "undral" {
				...
				global dbase "D:\Onedrive\OneDrive - ÐœÐžÐÐ“ÐžÐ› Ð£Ð›Ð¡Ð«Ð Ò®ÐÐ”Ð­Ð¡ÐÐ˜Ð™ Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð™Ð Ð¥ÐžÐ ÐžÐž\10.Personnal\6.Projects_with_others\FAO_Project\NDC\input"
	}

global data2018 "D:\Onedrive\OneDrive - ÐœÐžÐÐ“ÐžÐ› Ð£Ð›Ð¡Ð«Ð Ò®ÐÐ”Ð­Ð¡ÐÐ˜Ð™ Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð™Ð Ð¥ÐžÐ ÐžÐž\1.HSES\HSES_result_2018\HSES_2018"

global workdata_other "D:\Onedrive\OneDrive - ÐœÐžÐÐ“ÐžÐ› Ð£Ð›Ð¡Ð«Ð Ò®ÐÐ”Ð­Ð¡ÐÐ˜Ð™ Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð™Ð Ð¥ÐžÐ ÐžÐž\10.Personnal\6.Projects_with_others\FAO_Project\NDC\sattar_2018"
...
* Undral MAC
		...
		// Undral - mac
		if "`c(username)'" == "undral" {
				global base "//Users/undral/Library/CloudStorage/OneDrive-ÐœÐžÐÐ“ÐžÐ›Ð£Ð›Ð¡Ð«ÐÒ®ÐÐ”Ð­Ð¡ÐÐ˜Ð˜Ì†Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð˜Ì†ÐÐ¥ÐžÐ ÐžÐž/10.Personnal/6.Projects_with_others/FAO_Project/NDC" 
				global dbase "$base/input"
			}	
	
	
global data2018 "/Users/undral/Library/CloudStorage/OneDrive-ÐœÐžÐÐ“ÐžÐ›Ð£Ð›Ð¡Ð«ÐÒ®ÐÐ”Ð­Ð¡ÐÐ˜Ð˜Ì†Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð˜Ì†ÐÐ¥ÐžÐ ÐžÐž/1.HSES/HSES_result_2018/HSES_2018"
	
global workdata_other  "/Users/undral/Library/CloudStorage/OneDrive-ÐœÐžÐÐ“ÐžÐ›Ð£Ð›Ð¡Ð«ÐÒ®ÐÐ”Ð­Ð¡ÐÐ˜Ð˜Ì†Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð˜Ì†ÐÐ¥ÐžÐ ÐžÐž/10.Personnal/6.Projects_with_others/FAO_Project/NDC/sattar_2018"
...
** there is an error in q0108a in three cases, so repalcing zero with with 1 
	...
	*** Adding visitors
	...
	joinby hses_id using "$data2018/01_hhold", unm(b)
	drop _m
	rename hses_id identif 
...
joinby identif using "$data2018/workdata/consumption", unm(b)
keep if _m==3
drop _m
rename identif hhcode
...
gen fd_mv=daily_p_exp  // 20260210-d temdeglew aldaatai bn flag==. ued bh baij 
...
*joinby item_cod using "D:\18.FAO\PoU_Training_4-8_NOV_2019\POU data\NCT_Mongolia", unm(b)
rename item_cod itemcode

*joinby itemcode using country_nct_2018, unm(b)
*joinby itemcode using country_nct_2020_0211, unm(b)
*joinby itemcode using "D:\Onedrive\OneDrive - ÐœÐžÐÐ“ÐžÐ› Ð£Ð›Ð¡Ð«Ð Ò®ÐÐ”Ð­Ð¡ÐÐ˜Ð™ Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð™Ð Ð¥ÐžÐ ÐžÐž\3.NSO\3.Other_inNSO\1.EZSG\18.FAO\Before_2021\Adept\2018\Country_nct_2020_2018_with_Foodout", unm(b)

joinby itemcode using "/Users/undral/Library/CloudStorage/OneDrive-ÐœÐžÐÐ“ÐžÐ›Ð£Ð›Ð¡Ð«ÐÒ®ÐÐ”Ð­Ð¡ÐÐ˜Ð˜Ì†Ð¡Ð¢ÐÐ¢Ð˜Ð¡Ð¢Ð˜ÐšÐ˜Ð˜Ì†ÐÐ¥ÐžÐ ÐžÐž/3.NSO/3.Other_inNSO/1.EZSG/18.FAO/Before_2021/Adept/2018/Country_nct_2020_2018_with_Foodout", unm(b)

rename Calories calories_2Sonia
rename fd_kcal kcal
*gen kcal=4*fd_pro+4*fd_car+9*fd_fat+2*fd_fib

tab itemcode if _m==1
tab _m 
drop if _m==2
drop _m
...
collapse (mean) urban region decile hh_size tcal tth_inc f_exp hhweight hhweight_adj month,  by (hhcode)
count
...
preserve 
	...
	collapse (sd) energyforSD (mean) tcal adj_energy (first) sample_HH sample_pop repsentative_pop [fweight = round(hhweight_adj*hh_size)], by(urban)	
	tempfile urban
	save `urban', replace 
restore
...
preserve 
	...
	gen energyforSD=adj_energy
	collapse (sd) energyforSD (mean) tcal adj_energy (first) sample_HH sample_pop repsentative_pop [fweight = round(hhweight_adj*hh_size)], by(region)
	tempfile region
	save `region', replace 
restore
...
gen energyforSD=adj_energy
collapse (sd) energyforSD (mean) tcal adj_energy (first) sample_HH sample_pop repsentative_pop [fweight = round(hhweight_adj*hh_size)]
...
export excel using "$workdata_other/Results_stata_2018_20200421_drop21.csv", sheetreplace firstrow (variables)
*export excel using "$workdata_other/2018/Results_stata_2018_20200411_computedkcal.csv", sheetreplace firstrow (variables)

save "$workdata_other/result_file_2018_202004421_drop21", replace
